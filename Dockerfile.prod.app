# ---------------------------------------------------
# Build Stage 0 | Running Unit and Feature Tests
#----------------------------------------------------
...
RUN echo "APP_KEY=" >> .env && echo "APP_DEBUG=true" >> .env && php artisan key:generate
...
# ---------------------------------------------------
# Build Stage 2 | Build NPM production assets
#----------------------------------------------------
FROM node:12-alpine AS stage2
WORKDIR /var/www
COPY --from=stage1 /var/www /var/www
RUN rm -rf public/css && rm -rf public/js && rm -rf public/img
RUN npm install npm -g && npm install && npm run production && rm -rf node_modules



# --------------------------------------------------------
# Build Stage 3 | Build PHP code to production - PHP-FPM
#---------------------------------------------------------
FROM php:7.4.24-fpm AS stage3
WORKDIR /var/www
